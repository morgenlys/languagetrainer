...
export const shuffle = (arr) => { for (let i=arr.length-1;i>0;i--){ const j=randInt(i+1); [arr[i],arr[j]]=[arr[j],arr[i]];} return arr; };
...
export function isAcceptable(user, expected, alts = []){
  ...
  if (candidates.includes(nUser)) return {ok:true, distance:0};
  ...
  return {ok: best <= threshold, distance: best};
}
...
export function parseCSV(text){
  ...
  const data = rows.map(r => {
    ...
    return {
      ...
      tags, notes: r.notes || '', tokens_fr: tokens
    };
  });
...
export const uniqueText = (arr) => uniqueBy(arr, o => normalize(o.text));
