...
export function newProgressFor(items){
  ...
  for (const it of items){
    map[it.id] = {
      id: it.id, seen:0, correct:0, wrong:0, streak:0,
      stage:0, ef:2.5, interval:0, due:0, typingUnlocked:false, modeHistory:[]
    };
  }
  return map;
}
...
export function mergeProgress(items, progress){
  ...
  for (const id of Object.keys(progress)){ if(!ids.has(id)) delete progress[id]; }
  for (const it of items){ if(!progress[it.id]) progress[it.id] = newProgressFor([it])[it.id]; }
  return progress;
}

function scheduleNext(p, grade){ // 0..5
  const dnow = now();
  if (grade >= 3){
    ...
    p.interval = 0; p.due = dnow + 12*60*60*1000;
  }
}

export function updateAfterAnswer(p, wasCorrect, modeId){
  p.seen++; p.modeHistory.push({t: now(), mode: modeId, ok: wasCorrect});
...
export function selectNextItem(items, progress, {onlyDue, includeWords, includeSentences}){
  ...
  for (const s of scored){ ticket -= s.score; if (ticket <= 0) return s.it; }
  return choice(pool);
}

export function allowedModesFor(p){
  const modes = [];
  if (p.stage === 0){ modes.push('mc_df'); }
  else if (p.stage === 1){ modes.push('mc_fd','mc_df'); }
  else { modes.push('mc_df','mc_fd','input_df','match5','speech_mc','speech_input','sentence_build'); }
  return modes;
}
...
export function distractorsFor(items, target, n=3, direction='df'){
  ...
  const preferred = pool.filter(x => x.tags?.some(t => target.tags?.includes(t)));
  const rest = pool.filter(x => !preferred.includes(x));
  const cand = shuffle(preferred).concat(shuffle(rest));
  const out = [];
  const seen = new Set([normalize(target[key])]); // verhindert Duplikate inkl. korrekter Text
  for (const x of cand){
    const t = normalize(x[key]);
    if (seen.has(t)) continue;
    out.push({id:x.id, text:x[key]});
    seen.add(t);
